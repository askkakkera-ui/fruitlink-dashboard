        ...
        <div style={{ marginBottom: 4 }}>
          {!gpsResult && !gpsMsg && (
            <button type="button" onClick={startGps}
              style={{ width: '100%', padding: '12px', borderRadius: 10, border: '2px solid #FE6505', background: '#FFF3EA', color: '#FE6505', fontSize: 15, fontWeight: 700, cursor: 'pointer', marginTop: 6 }}>
              📍 Tap to capture my location
            </button>
          )}
          {gpsMsg && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6, flexWrap: 'wrap', padding: '8px 12px', borderRadius: 8, background: gpsResult ? '#e7f8ef' : '#fdeeee' }}>
              <span style={{ fontSize: 12, color: gpsResult ? '#198754' : '#B42318', flex: 1 }}>{gpsMsg}</span>
              {!gpsResult && <button type="button" onClick={startGps} style={{ fontSize: 12, padding: '5px 12px', borderRadius: 6, border: 'none', background: '#1F2533', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>🔄 Retry</button>}
            </div>
          )}
        </div>
