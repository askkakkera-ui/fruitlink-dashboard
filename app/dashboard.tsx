...
function ThresholdsSection({ SB_URL, SB_KEY, showSaved, showErr, saving, setSaving, saved }: any) {
  ...
  const [thresholds, setThresholds] = useState<Record<string, any>>({})
  useEffect(() => {
    fetch(SB_URL + '/rest/v1/machines?select=id,display_name,sn,status', { headers: { apikey: SB_KEY, Authorization: 'Bearer ' + SB_KEY } })
      .then(r => r.json()).then(d => {
        if (Array.isArray(d)) {
          setMachines(d)
          const t: Record<string, any> = {}
  ...
  const save = async () => { setSaving(true); showSaved(); setSaving(false) }
  return (
    <div>
      <div style={{ fontSize: 15, fontWeight: 600, color: C.text, marginBottom: 4 }}>Thresholds</div>
      <div style={{ fontSize: 13, color: C.text2, marginBottom: 22 }}>Temperature alert thresholds per machine</div>
      {machines.map((m: any) => (
        <div key={m.id} style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 12, padding: '16px 20px', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: m.status === 'online' ? C.greenBg : C.redBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: 14 }}>{m.status === 'online' ? '🟢' : '🔴'}</span>
            </div>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{m.display_name}</div>
              <div style={{ fontSize: 10, color: C.text3, fontFamily: 'monospace' }}>{m.sn}</div>
            </div>
            <Pill color={m.status === 'online' ? C.green : C.red} bg={m.status === 'online' ? C.greenBg : C.redBg} style={{ marginLeft: 'auto' }}>{m.status === 'online' ? 'Online' : 'Offline'}</Pill>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
            {[['Temp high alert (C)', 'temp_high', 'Alert when above'], ['Temp low alert (C)', 'temp_low', 'Alert when below'], ['Temp stop selling (C)', 'temp_stop', 'Stop vending above']].map(([label, key, desc]) => (
              <div key={key}>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: C.text2, marginBottom: 5, textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>{label}</label>
                <input type="number" value={thresholds[m.id]?.[key] ?? ''} onChange={e => setThresholds({ ...thresholds, [m.id]: { ...thresholds[m.id], [key]: +e.target.value } })}
                  style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid ' + C.border, fontSize: 13, outline: 'none', color: C.text, boxSizing: 'border-box' as const, background: C.surface2 }} />
                <div style={{ fontSize: 10, color: C.text3, marginTop: 3 }}>{desc}</div>
              </div>
            ))}
          </div>
        </div>
      ))}
      <button onClick={save} disabled={saving} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: C.orange, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saved ? '✓ Saved!' : 'Save thresholds'}</button>
    </div>
  )
}

function NotificationsSection({ operatorId, SB_URL, SB_KEY, showSaved, showErr, saving, setSaving, saved }: any) {
  const [phone, setPhone] = useState('+91 89771 10142')
  const [alerts, setAlerts] = useState<Record<string, boolean>>({
    machine_offline: true, temperature_high: true, temperature_low: true,
    temperature_stop: true, stock_empty: true, stock_low: false,
    door_open: true, vend_failure: true, cup_empty: true, film_empty: true,
  })
  const ALERT_INFO: any = {
    machine_offline: ['Machine offline', 'CRITICAL'],
    temperature_high: ['High temperature', 'CRITICAL'],
    temperature_low: ['Low temperature', 'HIGH'],
    temperature_stop: ['Temp stop selling', 'CRITICAL'],
    stock_empty: ['Stock empty (all layers)', 'HIGH'],
    stock_low: ['Stock low', 'MEDIUM'],
    door_open: ['Door open', 'HIGH'],
    vend_failure: ['Vend failure', 'HIGH'],
    cup_empty: ['Cups empty', 'HIGH'],
    film_empty: ['Film empty', 'HIGH'],
  }
  const SEV_COLOR: any = { CRITICAL: C.red, HIGH: C.amber, MEDIUM: C.blue }
  const SEV_BG: any = { CRITICAL: C.redBg, HIGH: C.amberBg, MEDIUM: C.blueBg }
  const save = async () => { setSaving(true); showSaved(); setSaving(false) }
  return (
    <div>
      <div style={{ fontSize: 15, fontWeight: 600, color: C.text, marginBottom: 4 }}>Notifications</div>
      <div style={{ fontSize: 13, color: C.text2, marginBottom: 22 }}>Control which alerts are sent to your WhatsApp</div>
      <div style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 12, padding: '16px 20px', marginBottom: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 10 }}>WhatsApp number</div>
        <input value={phone} onChange={e => setPhone(e.target.value)}
          style={{ width: '100%', maxWidth: 280, padding: '9px 12px', borderRadius: 8, border: '1px solid ' + C.border, fontSize: 13, outline: 'none', color: C.text, boxSizing: 'border-box' as const, background: C.surface2 }} />
        <div style={{ fontSize: 11, color: C.text3, marginTop: 6 }}>Powered by Twilio WhatsApp · Sandbox limit: 50 msg/day</div>
      </div>
      <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 12 }}>Alert types</div>
      <div style={{ border: '1px solid ' + C.border, borderRadius: 12, overflow: 'hidden' }}>
        {Object.entries(alerts).map(([key, val], i) => {
          const [label, sev] = ALERT_INFO[key] || [key, 'HIGH']
          return (
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', background: i % 2 === 0 ? C.surface : C.surface2, borderBottom: i < Object.keys(alerts).length - 1 ? '1px solid ' + C.border : 'none' }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, color: C.text, fontWeight: 500 }}>{label}</div>
                <div style={{ fontSize: 10, color: C.text3, marginTop: 1, fontFamily: 'monospace' }}>{key}</div>
              </div>
              <Pill color={SEV_COLOR[sev]} bg={SEV_BG[sev]}>{sev}</Pill>
              <div onClick={() => setAlerts({ ...alerts, [key]: !val })} style={{
                width: 36, height: 20, borderRadius: 10, background: val ? C.orange : C.border2,
                cursor: 'pointer', position: 'relative' as const, transition: 'background .2s', flexShrink: 0
              }}>
                <div style={{ position: 'absolute' as const, top: 2, left: val ? 18 : 2, width: 16, height: 16, borderRadius: '50%', background: '#fff', transition: 'left .2s' }} />
              </div>
            </div>
          )
        })}
      ...
      <div style={{ marginTop: 16 }}>
        <button onClick={save} disabled={saving} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: C.orange, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saved ? '✓ Saved!' : 'Save notifications'}</button>
      </div>
    </div>
  )
}
...
function CooldownsSection({ showSaved }: any) {
  const COOLDOWNS = [
    { type: 'machine_offline', label: 'Machine offline', hours: '1h', sev: 'CRITICAL' },
    { type: 'temperature_high', label: 'High temperature', hours: '1h', sev: 'CRITICAL' },
    { type: 'temperature_low', label: 'Low temperature', hours: '2h', sev: 'HIGH' },
    { type: 'temperature_stop', label: 'Temp stop selling', hours: '1h', sev: 'CRITICAL' },
    { type: 'stock_empty_l1', label: 'Layer 1 empty', hours: '4h', sev: 'HIGH' },
    { type: 'stock_empty_l2', label: 'Layer 2 empty', hours: '4h', sev: 'HIGH' },
    { type: 'stock_empty_l3', label: 'Layer 3 empty', hours: '4h', sev: 'HIGH' },
    { type: 'stock_low_l1', label: 'Layer 1 low', hours: '6h', sev: 'MEDIUM' },
    { type: 'stock_low_l2', label: 'Layer 2 low', hours: '6h', sev: 'MEDIUM' },
    { type: 'stock_low_l3', label: 'Layer 3 low', hours: '6h', sev: 'MEDIUM' },
    { type: 'door_open', label: 'Door open', hours: '1h', sev: 'HIGH' },
    { type: 'vend_failure', label: 'Vend failure', hours: '30m', sev: 'HIGH' },
    { type: 'cup_empty', label: 'Cups empty', hours: '2h', sev: 'HIGH' },
    { type: 'film_empty', label: 'Film empty', hours: '2h', sev: 'HIGH' },
  ]
  const SEV_COLOR: any = { CRITICAL: C.red, HIGH: C.amber, MEDIUM: C.blue }
  const SEV_BG: any = { CRITICAL: C.redBg, HIGH: C.amberBg, MEDIUM: C.blueBg }
  return (
    <div>
      <div style={{ fontSize: 15, fontWeight: 600, color: C.text, marginBottom: 4 }}>Cooldowns</div>
      <div style={{ fontSize: 13, color: C.text2, marginBottom: 22 }}>Minimum time before the same alert fires again</div>
      <div style={{ border: '1px solid ' + C.border, borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 80px', padding: '10px 16px', background: C.surface2, borderBottom: '2px solid ' + C.border }}>
          {['Alert type', 'Severity', 'Cooldown'].map(h => (
            <div key={h} style={{ fontSize: 10, fontWeight: 700, color: C.text2, textTransform: 'uppercase' as const, letterSpacing: '0.07em' }}>{h}</div>
          ))}
        </div>
        {COOLDOWNS.map((row, i) => (
          <div key={row.type} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 80px', padding: '11px 16px', background: i % 2 === 0 ? C.surface : C.surface2, borderBottom: i < COOLDOWNS.length - 1 ? '1px solid ' + C.border : 'none', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: 13, color: C.text, fontWeight: 500 }}>{row.label}</div>
              <div style={{ fontSize: 10, color: C.text3, fontFamily: 'monospace', marginTop: 1 }}>{row.type}</div>
            </div>
            <div><Pill color={SEV_COLOR[row.sev]} bg={SEV_BG[row.sev]}>{row.sev}</Pill></div>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.text, fontFamily: 'monospace' }}>{row.hours}</div>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 12, padding: '12px 16px', background: C.surface2, border: '1px solid ' + C.border, borderRadius: 10, fontSize: 12, color: C.text2 }}>
        Edit in <code style={{ fontFamily: 'monospace', background: C.border, padding: '1px 6px', borderRadius: 4 }}>/root/fruitlink/machine-api/alert.js</code> then run <code style={{ fontFamily: 'monospace', background: C.border, padding: '1px 6px', borderRadius: 4 }}>pm2 restart 2</code>
      </div>
    </div>
  )
}
...
function BillingSection({ role }: any) {
  return (
    <div>
      <div style={{ fontSize: 15, fontWeight: 600, color: C.text, marginBottom: 4 }}>Billing</div>
      <div style={{ fontSize: 13, color: C.text2, marginBottom: 22 }}>Manage your Fruitlink subscription and payments</div>
      <div style={{ border: '2px solid ' + C.orange, borderRadius: 14, padding: '20px 24px', marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.orange, textTransform: 'uppercase' as const, letterSpacing: '0.08em', marginBottom: 6 }}>Current plan</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: C.text }}>Fruitlink Pro</div>
            <div style={{ fontSize: 13, color: C.text2, marginTop: 4 }}>Full access · Unlimited machines · Real-time alerts</div>
          </div>
          <Pill color={C.green} bg={C.greenBg}>Active</Pill>
        </div>
        <div style={{ borderTop: '1px solid ' + C.border, paddingTop: 16, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
          {[['Machines', '2 / Unlimited'], ['Operators', '3 / Unlimited'], ['Alerts / month', 'Unlimited']].map(([l, v]) => (
            <div key={l}>
              <div style={{ fontSize: 10, color: C.text3, textTransform: 'uppercase' as const, letterSpacing: '0.06em', marginBottom: 4 }}>{l}</div>
      ...
      <div style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 12, padding: '16px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 36, height: 36, borderRadius: 8, background: C.orangeBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>💳</div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>Razorpay integration</div>
            <div style={{ fontSize: 12, color: C.text2, marginTop: 2 }}>Automated monthly billing, invoices, and payment history</div>
          </div>
          <Pill color={C.orange} bg={C.orangeBg} style={{ marginLeft: 'auto', flexShrink: 0 }}>Coming soon</Pill>
        </div>
      </div>
    </div>
  )
