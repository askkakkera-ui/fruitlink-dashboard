...
// ─── Machine Row (used in both operator and super admin views) ───
...
function MachineGroupedList({ machines, search, expandedId, setExpandedId, stockData, role, canEdit, openEdit, fmtTime, getCoords }: any) {
  ...
  // Super admin: group by operator → location (two-level)
  // Operator: group by location only (they only see their own machines)
  const isSuperAdmin = role === 'super_admin'

  // For operators — location-only grouping
  if (!isSuperAdmin) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {sortedLocs.map(([locName, locMachines]) => (
          <div key={locName} style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 16, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: C.surface2, borderBottom: '1px solid ' + C.border }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: C.text2 }}>📍 {locName}</span>
              <span style={{ fontSize: 12, color: C.text3, background: C.surface, padding: '2px 8px', borderRadius: 20, border: '1px solid ' + C.border }}>{locMachines.length} machine{locMachines.length !== 1 ? 's' : ''}</span>
              <span style={{ fontSize: 12, color: C.green }}>{locMachines.filter((m: any) => m.status === 'online').length} online</span>
            </div>
            {locMachines.map((m: any) => <MachineRow key={m.id} m={m} expandedId={expandedId} setExpandedId={setExpandedId} stockData={stockData} canEdit={canEdit} openEdit={openEdit} fmtTime={fmtTime} getCoords={getCoords} />)}
          </div>
        ))}
      </div>
    )
  ...
  // Super admin: group by operator → location
