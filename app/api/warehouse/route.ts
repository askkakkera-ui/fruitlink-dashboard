...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sp.get('buyers') === '1') {
      ...
      const r = await fetch(SB_URL + '/rest/v1/operators?select=id,name,email,company_name,billing_address,gstin,pincode,phone&role=eq.operator&deleted_at=is.null&order=name.asc', { headers: sbHeaders() });
      const rows = await r.json();
